...
// ==========================================
//    ROTARY ENCODER & MASTER VOLUME MIXER
// ==========================================
    wire [15:0] master_gain;
    wire [15:0] final_audio_sample;
    wire signed [31:0] mixed_sample;

    rotary_encoder master_vol_knob (
        .sysclk(clk_100), // Run on the same clock as the audio
        .encoder_a(enc_a),
        .encoder_b(enc_b),
        .encoder_sw(enc_sw),
        .led(led),        // The encoder directly drives the 4 LEDs now!
        .master_gain(master_gain)
    );

    // Multiply the raw music_player sample by the master_gain knob value.
    // We shift by 15 (take top bits) to scale it back down to a 16-bit range.
    assign mixed_sample = $signed(codec_sample) * $signed({1'b0, master_gain[14:0]});
    assign final_audio_sample = mixed_sample[30:15];
